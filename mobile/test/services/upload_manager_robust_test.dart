...
void main() {
  ...
  group('Robust Upload Manager Tests', () {
    test('handles corrupted box and recreates it', () async {
      ...
      await corruptFile.writeAsBytes([0xFF, 0xFF, 0xFF, 0xFF]); // Invalid Hive data
    ...
    test('retries initialization with exponential backoff', () async {
      // Temporarily make the box directory read-only to cause failures
      final hiveDir = Directory(tempDir.path);
      ...
      print('Debug state after init: $debugState');
    ...
    test('queues uploads when storage is temporarily unavailable', () async {
      ...
      try {
        ...
        print('Initial upload failed as expected: $e');
      }
    ...
    test('circuit breaker prevents excessive retries', () async {
      ...
      expect(debugState['circuitBreakerActive'], isFalse); // Should start inactive
    });
