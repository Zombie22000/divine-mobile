...
Future<String> createTestCacheDatabase(String testName, List<Map<String, String>> cacheEntries) async {
  ...
  for (final entry in cacheEntries) {
    await database.insert('cacheObject', {
      ...
      'validTill': DateTime.now().add(Duration(days: 30)).millisecondsSinceEpoch,
...
Future<void> createTestVideoFiles(String baseCacheDir, List<String> relativePaths) async {
...
void main() {
  ...
  group('VideoCacheManager initialization', () {
    ...
    tearDown(() async {
      ...
      final cacheDir = Directory(path.join(tempDir.path, VideoCacheManager.key));
      if (await cacheDir.exists()) {
    ...
    test('initialize() should populate cache manifest from database', () async {
      ...
      await createTestVideoFiles(baseCacheDir, ['video1.mp4', 'video2.mp4', 'subdir/video3.mp4']);

      // Mock the database path to use our test database
      final dbPath = await sqflite.getDatabasesPath();
      final testDbPath = path.join(dbPath, '${VideoCacheManager.key}_populate_test.db');
    ...
    test('initialize() should skip files that don\'t exist on filesystem', () async {
      ...
      final dbPath = await sqflite.getDatabasesPath();
      final testDbPath = path.join(dbPath, '${VideoCacheManager.key}_skip_missing_test.db');
      final expectedDbPath = path.join(dbPath, '${VideoCacheManager.key}.db');
    ...
    test('getCachedVideoSync() should return file when in manifest and exists', () async {
      ...
      // Set up database
      final dbPath = await sqflite.getDatabasesPath();
      final testDbPath = path.join(dbPath, '${VideoCacheManager.key}_sync_test.db');
      final expectedDbPath = path.join(dbPath, '${VideoCacheManager.key}.db');
    ...
    test('getCachedVideoSync() should return null when not in manifest', () async {
      ...
      final result = cacheManager.getCachedVideoSync('nonexistent_video_12345');
    ...
    test('getCachedVideoSync() should remove stale entry if file deleted', () async {
      ...
      // Set up database
      final dbPath = await sqflite.getDatabasesPath();
      final testDbPath = path.join(dbPath, '${VideoCacheManager.key}_stale_test.db');
      final expectedDbPath = path.join(dbPath, '${VideoCacheManager.key}.db');
      ...
      final beforeDelete = freshManager.getCachedVideoSync('stale_test_video');
  ...
  group('Video caching operations', () {
    ...
    test('cacheVideo() should add entry to manifest after successful cache', () async {
      ...
      // Set up database
      final dbPath = await sqflite.getDatabasesPath();
      final testDbPath = path.join(dbPath, '${VideoCacheManager.key}_cache_ops_test.db');
      final expectedDbPath = path.join(dbPath, '${VideoCacheManager.key}.db');
    ...
    test('isVideoCached() should update manifest when cache is found', () async {
      ...
      // Set up database
      final dbPath = await sqflite.getDatabasesPath();
      final testDbPath = path.join(dbPath, '${VideoCacheManager.key}_is_cached_test.db');
      final expectedDbPath = path.join(dbPath, '${VideoCacheManager.key}.db');
      ...
      final syncCheck = cacheManager.getCachedVideoSync('is_cached_test_video');
      expect(syncCheck, isNotNull);
      expect(syncCheck!.existsSync(), isTrue);
    ...
    test('getCachedVideo() should update manifest for synchronous lookups', () async {
      ...
      // Set up database
      final dbPath = await sqflite.getDatabasesPath();
      final testDbPath = path.join(dbPath, '${VideoCacheManager.key}_get_cached_test.db');
      final expectedDbPath = path.join(dbPath, '${VideoCacheManager.key}.db');
      ...
      final beforeInit = cacheManager.getCachedVideoSync('get_cached_test_video');
      ...
      final afterInit = cacheManager.getCachedVideoSync('get_cached_test_video');
    ...
    test('removeCorruptedVideo() should remove from manifest', () async {
      ...
      // Set up database
      final dbPath = await sqflite.getDatabasesPath();
      final testDbPath = path.join(dbPath, '${VideoCacheManager.key}_corrupted_test.db');
      final expectedDbPath = path.join(dbPath, '${VideoCacheManager.key}.db');
      ...
      final beforeRemove = cacheManager.getCachedVideoSync('corrupted_test_video');
      ...
      final afterRemove = cacheManager.getCachedVideoSync('corrupted_test_video');
    ...
    test('clearAllCache() should clear manifest', () async {
      ...
      final baseCacheDir = path.join(tempDir.path, VideoCacheManager.key);
      await createTestVideoFiles(baseCacheDir, ['clear1.mp4', 'clear2.mp4', 'clear3.mp4']);

      // Set up database
      final dbPath = await sqflite.getDatabasesPath();
      final testDbPath = path.join(dbPath, '${VideoCacheManager.key}_clear_test.db');
      final expectedDbPath = path.join(dbPath, '${VideoCacheManager.key}.db');
