...
void main() {
  group('Profile fetching with tab visibility', () {
    ...
    test('should allow profile fetching when Explore tab is active (index 2)', () {
      ...
      final canFetchProfiles = isFeedActive || isExploreActive || isProfileActive;
      
      expect(canFetchProfiles, isTrue,
        reason: 'Profile fetching should be allowed when Explore tab is active');
    });
    
    test('should NOT fetch profiles when Feed tab is active (index 0)', () async {
    ...
    test('should fetch profiles when any video tab is active', () async {
      ...
      expect(isFeedActive || isExploreActive || isProfileActive, isTrue,
    ...
    test('should NOT fetch profiles when camera tab is active (index 1)', () async {
      ...
      expect(isFeedActive || isExploreActive || isProfileActive, isFalse,
  ...
  group('ExploreScreen tab visibility synchronization', () {
    ...
    test('ExploreScreen.onScreenVisible should set tab visibility to index 2', () {
      ...
      // Assert
      expect(container.read(tabVisibilityProvider), equals(2),
    ...
    test('Profile fetching should work immediately after ExploreScreen becomes visible', () async {
      // Arrange - Start with camera tab (no profile fetching allowed)
      container.read(tabVisibilityProvider.notifier).setActiveTab(1);
      
      var canFetch = container.read(isFeedTabActiveProvider) || 
      ...
      canFetch = container.read(isFeedTabActiveProvider) || 
                 container.read(isExploreTabActiveProvider) || 
                 container.read(isProfileTabActiveProvider);
      expect(canFetch, isTrue,
