...
class P2PVideoSyncService extends ChangeNotifier {
  ...
  Future<void> startAutoSync({Duration interval = const Duration(minutes: 5)}) async {
    ...
    debugPrint('P2P Video Sync: Auto-sync started (interval: ${interval.inMinutes}m)');
  }
  ...
  Future<void> syncWithAllPeers() async {
    ...
    debugPrint('P2P Video Sync: Starting sync with ${connections.length} peers');
  ...
  Future<void> _syncWithPeer(P2PConnection connection) async {
    try {
      ...
      final syncMessage = {
        ...
        'video_summaries': localVideos.map((event) => {
          'id': event.id,
      ...
      debugPrint('P2P Video Sync: Synced with ${connection.peer.name} (${localVideos.length} videos)');
      
    } catch (e) {
      debugPrint('P2P Video Sync: Failed to sync with ${connection.peer.name}: $e');
    }
  }
  
  /// Send a sync message to a peer
  Future<void> _sendSyncMessage(P2PConnection connection, Map<String, dynamic> message) async {
  ...
  Future<void> handleIncomingSync(String peerId, Map<String, dynamic> message) async {
  ...
  Future<void> _handleSyncOffer(String peerId, Map<String, dynamic> message) async {
    ...
    if (missingVideos.isNotEmpty) {
      ...
      debugPrint('P2P Video Sync: Requested ${missingVideos.length} missing videos from $peerId');
    }
  }
  
  /// Handle sync request from a peer
  Future<void> _handleSyncRequest(String peerId, Map<String, dynamic> message) async {
    ...
    debugPrint('P2P Video Sync: Sent ${requestedVideos.length} videos to $peerId');
  }
  
  /// Handle video data from a peer
  Future<void> _handleVideoData(String peerId, Map<String, dynamic> message) async {
